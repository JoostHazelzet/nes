...
cdef class NESPPU:
    ...
    def __init__(self, cart=None, interrupt_listener=None, palette=None):
        ...
        cdef int[8][8] sprite_pattern = [[0 for _ in range(8)] for _ in range(8)]
        self._sprite_pattern = sprite_pattern
        cdef int[8] sprite_bkg_priority = [0 for _ in range(8)]
        self._sprite_bkg_priority = sprite_bkg_priority
    ...
    cdef void _fill_sprite_latches(self, int double_sprites):
        ...
        for i in range(self._num_active_sprites):
            ...
            for x in range(8):
                ...
                # JH: Added print to debug the int story in list
                if palette[c] > 255:
                    print(f"palette[{c}] = ", palette[c])
    ...
    cdef object get_ppu_state(self):
        ppu_state = {
            ...
            # "ppu_scroll": self.ppu_scroll, # Donkey kong doesn't use scrolling
            "ppu_addr": self.ppu_addr,
            "_ppu_byte_latch": self._ppu_byte_latch,
            "_ppu_data_buffer": self._ppu_data_buffer,
            "_io_latch": self._io_latch,
            "_palette": self._palette,
            "_pattern_lo": self._pattern_lo,
            "_pattern_hi": self._pattern_hi,
            "_sprite_pattern": self._sprite_pattern,
            "_sprite_bkg_priority": self._sprite_bkg_priority,
            "_num_active_sprites": self._num_active_sprites,
            "line": self.line,
            "pixel": self.pixel,
            "in_vblank": self.in_vblank,
            "sprite_zero_hit": self.sprite_zero_hit,
            "sprite_overflow": self.sprite_overflow,
            "ignore_ppu_ctrl": self.ignore_ppu_ctrl,
            "cycles_since_reset": self.cycles_since_reset,
            "frames_since_reset": self.frames_since_reset,
            "vram": [self.vram.read(a) for a in range(0x4000)],
            #"interrupt_listener": self.interrupt_listener, # function
            #"transparent_color": self.transparent_color, # fixed value
            "_palette_cache_valid": self._palette_cache_valid,
            "irq_tick_triggers": self.irq_tick_triggers
        }
        return ppu_state
    
    cdef void set_ppu_state(self, object state):
    
        self.ppu_ctrl = state["ppu_ctrl"]
        self.ppu_mask = state["ppu_mask"]
        self.oam_addr = state["oam_addr"]
        self.oam_data = state["oam_data"]
        
        self.ppu_addr = state["ppu_addr"]
        self._ppu_byte_latch = state["_ppu_byte_latch"]
        self._ppu_data_buffer = state["_ppu_data_buffer"]
        self._io_latch = state["_io_latch"]
        self._palette = state["_palette"]
        self._pattern_lo = state["_pattern_lo"]
        self._pattern_hi = state["_pattern_hi"]

        cdef int[8][8] sprite_pattern = [[0 for _ in range(8)] for _ in range(8)]
        for i in range(8):
            if (len(state["_sprite_pattern"][i]) % 4) == 0 and len(state["_sprite_pattern"][i]) > 0:
                int32_list = list(struct.unpack(f'{len(state["_sprite_pattern"][i])//4}i', state["_sprite_pattern"][i]))
                for j in range(len(int32_list)):
                    sprite_pattern[i][j] = int32_list[j]
        self._sprite_pattern = sprite_pattern

        cdef int[8] sprite_bkg_priority = [0 for _ in range(8)]
        if (len(state["_sprite_bkg_priority"]) % 4) == 0 and len(state["_sprite_bkg_priority"]) > 0:
            int32_list = list(struct.unpack(f'{len(state["_sprite_bkg_priority"])//4}i', state["_sprite_bkg_priority"]))
            for i in range(8):
                sprite_bkg_priority[i] = int32_list[i]
        self._sprite_bkg_priority = sprite_bkg_priority
        
        self._num_active_sprites = state["_num_active_sprites"]
        ...
        self.ignore_ppu_ctrl = state["ignore_ppu_ctrl"]
        self.cycles_since_reset = state["cycles_since_reset"]
        self.frames_since_reset = state["frames_since_reset"]

        for a in range(len(state["vram"])):
            self.vram.write(a, state["vram"][a])

        self._palette_cache_valid = state["_palette_cache_valid"]
        self.irq_tick_triggers = state["irq_tick_triggers"]
